...
-- theorem transition_project' [DecidableEq state](dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (m : Fin (n + 1)) (f : (Fin (n+1) → Fin (b+2))) (p q : state) : dfa.transition f p = q → q ∈ ((project dfa).transition (Matrix.vecTail f) p).val := by
...
theorem transition_project [DecidableEq state](dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (f : (Fin (n+1) → Fin (b+2))) (p : state) : dfa.transition f p ∈ ((project dfa).transition (Matrix.vecTail f) p).val := by
  simp only [project, List.mem_dedup, List.mem_map, FinEnum.mem_toList, true_and]
  use Matrix.vecHead f
  simp only [Matrix.cons_head_tail]

theorem transFrom_project [DecidableEq state](dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (l : List (Fin (n+1) → Fin (b+2))) (p q : state) : dfa.transFrom l p = q → q ∈ ((project dfa).transFrom (l.map (fun f => Matrix.vecTail f)) ⟨[p], by exact List.nodup_singleton p⟩).val := by
  intro h
  ...
  case cons f fs ih =>
    ...
    have := NFA.transFrom_sublist (ps := ⟨[dfa.transition f p], by exact List.nodup_singleton (dfa.transition f p)⟩) (project dfa) (List.map (fun f ↦ Matrix.vecTail f) fs) ⟨(((project dfa).transition (Matrix.vecTail f) p).val).dedup, by apply List.nodup_dedup⟩
    apply this
    . simp only [List.cons_subset, List.mem_dedup, List.nil_subset, and_true]
      apply transition_project
    . assumption

theorem evalFrom_project [DecidableEq state](dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (l : List (Fin (n+1) → Fin (b+2))) (p : state) : dfa.evalFrom l p → (project dfa).evalFrom (l.map (fun f => Matrix.vecTail f)) ⟨[p], by exact List.nodup_singleton p⟩ := by
...
theorem eval_project [DecidableEq state](dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (l : List (Fin (n+1) → Fin (b+2))) : dfa.eval l → (project dfa).eval (l.map (fun f => Matrix.vecTail f)) := by
  simp[NFA.eval, DFAO.eval]
  exact evalFrom_project dfa l dfa.start

-- theorem project_transition' [DecidableEq state](dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (m : Fin (n + 1)) (f : (Fin n → Fin (b+2))) (p q : state) : q ∈ ((project dfa).transition f p).val → ∃ f₁, f = Matrix.vecTail f₁ ∧ dfa.transition f₁ p = q := by
--   simp only [project, List.mem_dedup, List.mem_map, FinEnum.mem_toList, true_and]
--   intro h
--   rcases h with ⟨a, h'⟩
...
theorem project_transition [DecidableEq state](dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (f : (Fin n → Fin (b+2))) (p q : state) : q ∈ ((project dfa).transition f p).val → ∃ a, dfa.transition (Matrix.vecCons a f) p = q := by
  simp[project]

theorem project_transFrom [DecidableEq state](dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (l : List (Fin n → Fin (b+2))) (q : state) (states : ListND state) : q ∈ ((project dfa).transFrom l states).val → ∃ p ∈ states.val, ∃ (l₁ : List (Fin (n+1) → Fin (b+2))) , l = l₁.map (Matrix.vecTail) ∧ dfa.transFrom l₁ p = q:= by
  intro h
...
theorem project_evalFrom [DecidableEq state](dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (l : List (Fin n → Fin (b+2))) (states : ListND state) : (project dfa).evalFrom l states → ∃ p ∈ states.val, ∃ (l₁ : List (Fin (n+1) → Fin (b+2))) , l = l₁.map (Matrix.vecTail) ∧ dfa.evalFrom l₁ p:= by
  intro h
...
theorem project_eval [DecidableEq state](dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (l : List (Fin n → Fin (b+2))) : (project dfa).eval l → ∃ (l₁ : List (Fin (n+1) → Fin (b+2))) , l = l₁.map (Matrix.vecTail) ∧ dfa.eval l₁ := by
  simp[NFA.eval, DFAO.eval]
...
theorem project_eval_iff [DecidableEq state](dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (l : List (Fin n → Fin (b+2))) : (project dfa).eval l ↔ ∃ (l₁ : List (Fin (n+1) → Fin (b+2))) , l = l₁.map (Matrix.vecTail) ∧ dfa.eval l₁ := by
  constructor
...
theorem project_acceptZero [DecidableEq state] (dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (h: DFA.acceptZero dfa) : NFA.acceptZero (project dfa) := by
  simp[NFA.acceptZero]
  intro l h'
...
theorem project_fix_acceptZero [Fintype state][DecidableEq state] (dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (h: dfa.acceptZero) : (project dfa).fixLeadingZeros.acceptZero := by
  have: (project dfa).acceptZero := project_acceptZero dfa h
  exact NFA.fixLeadingZeroes.acceptZero (project dfa) this

theorem project_fix_respectZero [Fintype state][DecidableEq state] (dfa : DFA (Fin (n + 1) → Fin (b+2)) state) (h: dfa.respectZero) : (project dfa).fixLeadingZeros.respectZero := by
  rw[NFA.respectZero]
  intro l k
  constructor
  ...
  . intro h'
    ...
    have := (project dfa).bounded_accept l this (padZeros (Fintype.card (ListND state) + k) l)
...
theorem correct_project [Fintype state] [DecidableEq state] (v : Fin l → ℕ) (dfa : DFA (Fin (l+1) → Fin (b+2)) state) (hres: dfa.respectZero):
...
theorem project_correct [Fintype state] [DecidableEq state] (v : Fin l → ℕ) (dfa : DFA (Fin (l+1) → Fin (b+2)) state) (hres: dfa.respectZero):
  ...
  have htoNat: toNat (padZeros (Fintype.card (ListND state)) (toWord v b)) = toNat (List.map (Matrix.vecTail) w) := by
    rw[hlis]
...
theorem project_iff [Fintype state] [DecidableEq state] (v : Fin l → ℕ) (dfa : DFA (Fin (l+1) → Fin (b+2)) state) (hres: dfa.respectZero):
