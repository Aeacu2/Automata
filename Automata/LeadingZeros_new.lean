...
def NFA.respectZero [LinearOrder state] (nfa : NFA (Fin n → Fin (b+2)) state) : Prop := ∀ (x : List (Fin n → Fin (b+2))), ∀ m, nfa.eval x ↔ nfa.eval (padZeros m x)
...
def NFA.acceptZero [LinearOrder state] (nfa : NFA (Fin n → Fin (b+2)) state) : Prop := ∀ (x : List (Fin n → Fin (b+2))), (nfa.eval x → ∀ m, nfa.eval (padZeros m x))
...
theorem NFA.acceptZero_of_respectZero [LinearOrder state] (nfa : NFA (Fin n → Fin (b+2)) state) (h: nfa.respectZero) : nfa.acceptZero := by
  rw[NFA.acceptZero]
...
theorem DFAO.bounded_out [Fintype state] (dfao : DFAO (Fin n → Fin (b+2)) state out) (x : List (Fin n → Fin (b+2))) (o: out)(h: ∀ (x : List (Fin n → Fin (b+2))), dfao.eval x = o → (∀ m, dfao.eval (padZeros m x) = o)) : ∀ k, dfao.eval (padZeros k x) = o → dfao.eval (padZeros (Fintype.card state) x) = o := by
  ...
  . induction' hk : k - Fintype.card state using Nat.strong_induction_on  with d ih generalizing k
    ...
    have hac : a ++ c = padZeros (k - b'.length) x := by
      ...
      simp only [List.replicate_append_replicate, List.append_cancel_right_eq, List.replicate_inj,
        AddLeftCancelMonoid.add_eq_zero, List.length_eq_zero_iff, or_true, and_true]
      omega
...
theorem NFA.bounded_accept [Fintype state] [LinearOrder state] (nfa : NFA (Fin n → Fin (b+2)) state) (x : List (Fin n → Fin (b+2)))(h: nfa.acceptZero): ∀ z, (∃ k, z = padZeros k x) ∧ nfa.eval z → nfa.eval (padZeros (Fintype.card (ListSND state)) x) := by
...
def NFA.fixLeadingZeros [Fintype state] [LinearOrder state] (nfa : NFA (Fin n → Fin (b+2)) state) : NFA (Fin n → Fin (b+2)) state := {
  transition := nfa.transition
  start := nfa.transFrom (padZeros (Fintype.card (ListSND state)) []) nfa.start
  output := nfa.output
}

theorem NFA.fixLeadingZeros_transFrom [Fintype state] [LinearOrder state] (nfa : NFA (Fin n → Fin (b+2)) state) (s : List (Fin n → Fin (b+2))): (nfa.fixLeadingZeros).transFrom s nfa.fixLeadingZeros.start = nfa.transFrom (padZeros (Fintype.card (ListSND state)) s) nfa.start := by
...
theorem NFA.fixLeadingZeros_eval [Fintype state] [LinearOrder state] (nfa : NFA (Fin n → Fin (b+2)) state) (x : List (Fin n → Fin (b+2))) : (nfa.fixLeadingZeros).eval x = nfa.eval (padZeros (Fintype.card (ListSND state)) x) := by
  simp only [NFA.eval, NFA.evalFrom]
  rw[NFA.fixLeadingZeros_transFrom]
  rfl

theorem NFA.fixLeadingZeros.accept [Fintype state] [LinearOrder state] (nfa : NFA (Fin n → Fin (b+2)) state) (x : List (Fin n → Fin (b+2)))(h: nfa.acceptZero): ∀ z, (∃ k, z = padZeros k x) ∧ nfa.eval z → nfa.fixLeadingZeros.eval x := by
  intro z a
...
theorem NFA.fixLeadingZeroes.acceptZero [Fintype state] [LinearOrder state] (nfa : NFA (Fin n → Fin (b+2)) state) (h: nfa.acceptZero): (nfa.fixLeadingZeros).acceptZero := by
